(function(){
    ...
    node.getChildNodes = function(httpRequest, httpResponse){
        ...
        function addAllowedNodes(node, next){
            if(permissions.allowed(node._id.toString(), httpRequest.identity.role, httpRequest.identity.permissions, privileges.available.READER)){
                resp.push(node);
            }
            next();
        }
    ...
    node.attachAsset = function(httpRequest, httpResponse){
        ...
        request.parseForm(httpRequest).done(function(obj){
            ...
            if(obj.files){
                async.each(obj.files.file,saveAsset,done);
            }
        });
    };
    ...
    node.getAssetsDeep = function(httpRequest, httpResponse){
        ...
        var allowedNodes = [],
            ...
            nodes.getAssets(httpRequest.params.nodeid).then(function(obj){
                resp.push(obj);

                /*
                 Get all child nodes. then iterate to only include nodes that the user has access to.
                 */
                nodes.getChildNodes(httpRequest.params.nodeid, false).then(function(obj){
                    async.each(obj, function(node, next){
                        if(permissions.allowed(node._id, httpRequest.identity.role, httpRequest.identity.permissions, privileges.available.READER)){
                        ...
                        async.each(allowedNodes, function(node, next){
                            nodes.getAssets(node._id.toString()).then(function(obj){
                                resp.push(obj);
                                next();
                            });
                        },function(){
    ...
    node.getByIdDeep = function(httpRequest, httpResponse){
        ...
        nodes.getByIdDeep(httpRequest.params.nodeid)
            .then(function(obj){
                function checkPermissions(childnode, next){
                    if(permissions.allowed(childnode._id.toString(), httpRequest.identity.role, httpRequest.identity.permissions, privileges.available.READER)){
    ...
    node.getChildNodesDeep = function(httpRequest, httpResponse){
        ...
        nodes.getChildNodesDeep(httpRequest.params.nodeid)
            .then(function(obj){
