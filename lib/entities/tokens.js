...
module.exports = (function tokens(){
    ...
    function addSuper(token, profile){
        if(self.supers[token] == null){
            self.supers[token] = crypto.encrypt(profile);
        }
    }
    ...
    function addToken(token, profile){
        if(self.tokens[token] == null){
            self.tokens[token] = crypto.encrypt(profile);
        }
    }
    ...
    self.validate = function(token){
        ...
        if(self.tokens[token] != null){
            obj.isValid = true;
            obj.profile = crypto.decrypt(self.tokens[token]);
    ...
    self.create = function(token, profile){
         ...
         */
        if(profile.isSuper){
            addSuper(token, profile);
        }

        addToken(token, profile);
