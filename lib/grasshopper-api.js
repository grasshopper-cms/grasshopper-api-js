...
module.exports = function(options){
    ...
    function initialize(options, grasshopperInstance) {
        ...
        // Add Routes for the login view
        adminRouter.get('/login', require('./admin/client/views').renderLoginView);

        console.log('<<<<<<<<<< >>>>>>>>>>>>>>');
        grasshopper.event.channel('/system/db')
            .on('start', function(payload, next) {
                console.log('++++++ heard start event ++++++');
                console.log('options', options);
                grasshopper.auth('basic', {
                    username : options.grasshopperAdminUsername, password : options.grasshopperAdminPassword
                })
                    .then(function (token) {
                        console.log('call admin app init >>>>');
                        grasshopperInstance.request = grasshopper.request(token);
                        settingsPlugin.onAppInit(options, grasshopperInstance);
                        next();
                    })
                    .catch(function(e) { console.log('auth error', e); });
            });
    ...
    return {
        admin : initialize(options, { admin : adminRouter, core : grasshopper, bridgetown : bridgetown, router : ghapiRouter, state : {
