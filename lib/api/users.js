        ...
        base.authenticateRequest(req, res, function(token){
            base.userCan(token, base.AVAILABLE_PRIVILEGES.ADMIN, function(err){
                if(err){
                    ...
                    grasshopper.users.updatePermission(req.params.id, req.body, function(err, data){
                        if(err){
                            base.write(base.STATUS_CODES.SERVER_ERROR, JSON.stringify({message: err.message}), res);
                        }
                        else {
        ...
        base.authenticateRequest(req, res, function(token){
            base.userCan(token, base.AVAILABLE_PRIVILEGES.ADMIN, function(err){
                if(err){
                    ...
                    grasshopper.users.deletePermission(req.params.id, req.params.nodeid, function(err, data){
                        if(err){
                            base.write(base.STATUS_CODES.SERVER_ERROR, JSON.stringify({message: err.message}), res);
                        }
                        else {
