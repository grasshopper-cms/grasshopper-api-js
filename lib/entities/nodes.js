(function(){
    ...
    node.deleteById = function(id, callback){
        ...
        this.getByIdDeep(id, function(err, nodeList){
            if(err) { callback(err); return; }
console.log("deep node list");
            function deleteNode(node, cb){
                async.waterfall([
                    function(next){
                        console.log("deleting id: " + node._id.toString());
                        db.nodes.deleteById(node._id.toString(), next);
                    },
                    //function(next){
                    //TODO when content code is added then we also need to delete the content
                    //},
                    function(id, next){
                        console.log("deleting assets for  id: " + node._id.toString());
                        assets.deleteAll({nodeid: node._id.toString()}, next);
                    },
                    function(next){
                        console.log("deleting asset directory for  id: " + node._id.toString());
                        assets.removeDirectory({nodeid: node._id.toString()}, next);
                    }
                ],function(err, data){
