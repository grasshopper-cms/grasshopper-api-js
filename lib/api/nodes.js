(function(){
        ...
        base.authenticateRequest(req, res, function(token){
            userCan(token, base.AVAILABLE_PRIVILEGES.READER, req.params.id, function(err){
                if(err){
                    ...
                    nodes.getFiles(req.params.id, function(err, obj){
                        if(err){
                            if(err.message.indexOf('[404]') > -1){
                                base.write(base.STATUS_CODES.NOT_FOUND, JSON.stringify(err), res);
                            }
    ...
    node.attachFile = function(req, res){
        base.authenticateRequest(req, res, function(token){
                ...
                if(err){
                    ...
                    form.parse(req, function(err, fields, files) {
                        function saveFile(file, next){
                            nodes.saveFile(req.params.id, file.originalFilename, file.path, function(err){
                                next(err);
                            });
                        }
                        ...
                        if(files){
                            async.each(files.file,saveFile,done);
                        }
                    });
                }
            });
                        ...
                        nodes.getChildNodes(req.params.id, false, function(err, obj){
                            async.each(obj, function(node, next){
                                ...
                                async.each(allowedNodes, function(node, next){
                                    nodes.getFiles(node._id.toString(), function(err, obj){
                                        resp.push(obj);
                                        next();
                                    });
                                },function(){
