...
function _createSocialAccount(){
    return BB
        .bind(this)
        .then(function() {
            // gh returns a Q promise, that doesn't know about bind, so have to transform to a BB promise
...
function _linkIdentity() {
    ...
    return BB.resolve(grasshopper.request(this.token).users.linkIdentity(this.user._id.toString(), 'facebook', identityOptions));
}
