module.exports = function initUsers(app){
    ...
    function validateUser(userObj, cb){
        ...
        async.each(mandatoryProps, function(prop, callback) {
                ...
                else if(prop == "login" && (userObj[prop] == null || (userObj[prop] != null && userObj[prop].trim().length < 4))){
                    return callback(new Error("The login property cannot be null and has to have a minimum length of 4 characters."));
                }
