...
var permissions = function() {
     ...
     * @param next
     */
    function userHasPrivileges(permissionLevel, req) {
        var userPrivLevel = AVAILABLE_PRIVILEGES[req.identity.profile.role.toUpperCase()];

        return (userPrivLevel <= parseInt(permissionLevel, 10));
    }
    ...
    this.requireEditor = function(req, res, next){
        validateUserHandleResponse(AVAILABLE_PRIVILEGES.EDITOR, req, res, next);
    };

    this.requireAuthor = function(req, res, next){
        validateUserHandleResponse(AVAILABLE_PRIVILEGES.AUTHOR, req, res, next);
    };

    this.requireReader = function(req, res, next){
        validateUserHandleResponse(AVAILABLE_PRIVILEGES.READER, req, res, next);
    };
};
