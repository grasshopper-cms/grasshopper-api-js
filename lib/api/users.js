(function(){
    ...
    user.getCurrentUser = function (req, res){
        base.authenticateRequest(req, res, function(token){
            ...
            users.getById(userId, function(err, userObj){
                if(err){
        ...
        base.authenticateRequest(req, res, function(token){
            base.userCan(token, base.AVAILABLE_PRIVILEGES.ADMIN, function(err){
                if(err){
                    ...
                    users.getById(req.params.id, function(err, userObj){
                        if(err){
        ...
        base.authenticateRequest(req, res, function(token){
            base.userCan(token, base.AVAILABLE_PRIVILEGES.ADMIN, function(err){
                if(err){
                    ...
                    users.deleteById(req.params.id, function(err){
                        if(err){
                            base.write(base.STATUS_CODES.SERVER_ERROR, JSON.stringify(err), res);
                        }
                        else {
        ...
        base.authenticateRequest(req, res, function(token){
            base.userCan(token, base.AVAILABLE_PRIVILEGES.ADMIN, function(err){
                if(err){
                    ...
                    users.describe(function(err, userData){
                        if(err){
                            base.write(base.STATUS_CODES.SERVER_ERROR, JSON.stringify(err), res);
                        }
                        else {
        ...
        base.authenticateRequest(req, res, function(token){
            ...
            base.userCan(token, base.AVAILABLE_PRIVILEGES.ADMIN,function(err){
                ...
                if(err && (req.body._id != token.profile._id) ){
                    ...
                    users.update(req.body, function(err, userObj){
                        if(err){
        ...
        base.authenticateRequest(req, res, function(token){
            base.userCan(token, base.AVAILABLE_PRIVILEGES.ADMIN, function(err){
                if(err){
                    ...
                    users.updatePermission(req.params.id, req.body, function(err, data){
                        if(err){
                            base.write(base.STATUS_CODES.SERVER_ERROR, JSON.stringify({message: err.message}), res);
                        }
                        else {
                            base.write(base.STATUS_CODES.SUCCESS, JSON.stringify(data), res);
                        }
                    });
                }
            });
        ...
        base.authenticateRequest(req, res, function(token){
            base.userCan(token, base.AVAILABLE_PRIVILEGES.ADMIN, function(err){
                if(err){
                    ...
                    users.deletePermission(req.params.id, req.params.nodeid, function(err, data){
                        if(err){
                            base.write(base.STATUS_CODES.SERVER_ERROR, JSON.stringify({message: err.message}), res);
                        }
                        else {
