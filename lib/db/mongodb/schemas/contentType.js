module.exports = (function(){
    ...
    function isValidField(key, obj){
        ...
        if(!(obj instanceof Object)){
            err = new Error("Fields must be an instance of an object");
        }
        else {
            ...
            if(!err && key.indexOf(' ') >= 0){
                err = new Error("ids for fields cannot have any spaces.");
            }
            ...
            if(!err && !_.isNumber(obj.min) || !_.isNumber(obj.max)){
                err = new Error("'max / min' field value should be a number.");
            }

            if(!err && !_.isBoolean(obj.useAsLabel)){
                err = new Error("'useAsLabel' field value should be a boolean.");
            }

            if(!err && obj.helpText && !_.isString(obj.helpText)){
                err = new Error("'HelpText' field value should be a string.");
            }
    ...
    function validateFieldType(value){
        ...
        if(!_.findWhere(value, { useAsLabel : true })) {
            err = 'At least one field must be marked, Use As Label';
        }
