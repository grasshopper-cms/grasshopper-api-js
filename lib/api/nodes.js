module.exports = function(grasshopper){
        ...
        async.waterfall([
            ...
            function(user, permissions, cb){
                userCanForNode(user, parentNodeId, minRole, function(err){
                    //console.log(parentNodeId, minRole);
                    //console.log(err);
                ...
                if(err){
                    ...
                    grasshopper.nodes.getChildNodes(req.params.id, false, function(err, obj){
                        ...
                        async.each(obj, function(node, next){
                            ...
                            base.write(base.STATUS_CODES.SUCCESS, JSON.stringify(nodes), res);
                        });
                    });
                }
            });
        ...
        base.authenticateRequest(req, res, function(token){
            userCan(token, base.AVAILABLE_PRIVILEGES.READER, req.params.id, function(err){
                ...
                if(err){
                    ...
                    grasshopper.nodes.getFiles(req.params.id, function(err, obj){
                        if(err){
                            if(err.message.indexOf('[404]') > -1){
                                base.write(base.STATUS_CODES.NOT_FOUND, JSON.stringify(err), res);
                            }
    ...
    node.getFilesDeep = function(req, res){
        base.authenticateRequest(req, res, function(token){
            /*userCan(token, base.AVAILABLE_PRIVILEGES.READER, req.params.id, function(err){
