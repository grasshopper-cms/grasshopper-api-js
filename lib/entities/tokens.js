...
module.exports = (function tokens(){
     ...
     */
    function addSuper(token, userId){
        if(self.supers[token] == null){
            self.supers[token] = crypto.encrypt(userId);
        }
    }
    ...
    function addToken(token, userId){
        if(self.tokens[token] == null){
            self.tokens[token] = crypto.encrypt(userId);
        }
    }
    ...
    self.validate = function(token){
        ...
        if(self.tokens[token] != null){
            obj.isValid = true;
            obj.userId = crypto.decrypt(self.tokens[token]);
     ...
     * @returns {token|isValid|isSuper|userId}
     */
    self.create = function(userId, isSuper){
        var t = "1232131"; //[TODO] Need to create a guid for the token.
