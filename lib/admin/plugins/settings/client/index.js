'use strict';
gh.api.pagePartials
    .get('/admin/settings/html/root')
    .then(function(body) {
        window.plugins = body.plugins;
        window.tabs = body.tabs;

        var Vue = require('vue');
        var App = require('./template/template.vue');
        var app = new Vue({
            el: '#stage',
        ...
        function init() {
            view.activeSection = window.gh.util.location.getCurrentQueryParams().tab || view.activeSection;
        ...
        function handleSubSectionClicked(event) {
            window.gh.util.location.updateQueryParams({ tab : event.currentTarget.getAttribute('section-name') });
