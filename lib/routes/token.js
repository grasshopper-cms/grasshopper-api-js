...
module.exports = function(app){
        ...
        async.waterfall([
            ...
            function(err, accessToken){
                if(err){
                    ...
                    httpRequest.session.token = new Buffer(accessToken).toString('base64');
                    httpRequest.session.save(function(err){
                        if(!err){
                            response.writeSuccess({
                                'access_token': httpRequest.session.token,
                                'token_type': 'Token'
                            });
                        }
                        else {
    ...
    token.getNew = function(httpRequest, httpResponse){
        ...
        function writeResponse(token){
            ...
            httpRequest.session.token = token.access_token; // Yes this is supposed to be httpResponse.
            httpRequest.session.save(function(err){
                if(!err){
