module.exports = function(router){
    ...
    node.attachAsset = function(httpRequest, httpResponse){
        ...
        request.parseForm(httpRequest).then(function(form) {
            if(form.files) {
                BB.all(form.files.file.map(function(file) {
                    return grasshopper.request(httpRequest.bridgetown.token).assets.save({
                        nodeid: httpRequest.params.nodeid,
