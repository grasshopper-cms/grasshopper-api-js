...
module.exports = function(options){
    ...
    function initializeGhAdmin(options, grasshopperInstance) {
        ...
        // This is important so that the Headerbar, GH Admin Core etc are available to the plugins.
        adminRouter.use(express.static(path.join(__dirname, 'admin', 'public')));

        // THis is for the Grasshopper API....should happen later? Or should not be here?
        ghapiRouter.route('*').all(middlewares.concat([
            methodOverride(),
            bodyParser.json(bodyParserOptions),
            bodyParser.urlencoded(bodyParserOptions),
        ...
        // Attach Grasshopper Admin Configs to all admin routes.
        adminRouter.route('*').all([
            function(request, response, next) {
                response.locals.ghAdminConfigs = {
                    apiEndpoint : '/api',
                    buildDirectory : 'app/public/admin',
                    base : '/admin',
                    homeString : 'Grasshopper',
                    menuItems : [], // Deprecated
                    version : 'FROZEN SINCE 0.40.2 OLD ADMIN',
                    libraryVersions : [],
        ...
        adminRouter.route(['/', '/admin']).get(grasshopperInstance.renderAdminRoot);

        // adminRouter.get(['/login', '/login*'], require('./admin/views').renderOldAdmin);
        // adminRouter.get(['/logout', '/logout*'], require('./admin/views').renderOldAdmin);
        // adminRouter.get(['/advanced-search', '/advanced-search*'], require('./admin/views').renderOldAdmin);
        // adminRouter.get(['/info', '/sys-info*'], require('./admin/views').renderOldAdmin);
        // adminRouter.get(['/user', '/user*'], require('./admin/views').renderOldAdmin);
        // adminRouter.get(['/add-user', '/add-user*'], require('./admin/views').renderOldAdmin);
        // adminRouter.get(['/item', '/item*'], require('./admin/views').renderOldAdmin);
        // adminRouter.get(['/forbidden', '/forbidden*'], require('./admin/views').renderOldAdmin);
        // adminRouter.get(['/not-found', '/not-found*'], require('./admin/views').renderOldAdmin);
        ...
        admin : initializeGhAdmin(options,
            {
                ...
                renderAdminRoot : function(request, response) {
                    response.render(require.resolve('./admin/views/admin.pug'), response.locals);
                }
            }),
        core : grasshopper,
        bridgetown : bridgetown,
