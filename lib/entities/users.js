module.exports = function(app){
    ...
    var descriptionMapFunction = function (){
        var key = "users";
        var value = {
            count: 1,
    ...
    var reduceFunction = function(key, values) {
        ...
        values.forEach( function(value) {
                reducedObject.count ++;
                reducedObject.enabled += (value.enabled) ? 1 : 0;
                reducedObject.disabled += (value.disabled) ? 0 : 1;
            }
        );
    ...
    users.create = function(userObj, callback){
        ...
        validateUser(userObj,function(err){
            if(err){
                ...
                internal.app.db.users.getByLogin(userObj.login, function(err, val){
                    ...
                    if(err) {
                        ...
                        callback.call(this, new Error("User with the same login already exists."));
                    }
                });
    ...
    users.update = function(userObj, callback){
        validateUser(userObj,function(err){
            if(err){
                ...
                internal.app.db.users.update(userObj, function(err){
                    callback.call(this, err);
                });
            }
        });
